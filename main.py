    ...
    if ((name.isalpha()) or (' ' in name)):
        ...
        while (True):
            ...
            for (x, y, w, h) in faces:
                ...
                cv.imwrite("TrainingImage\ " + name + "." + str(serial) + "." + Id + '.' + str(sampleNum) + ".jpg",
                            gray[y:y + h, x:x + w])
                # display the frame
                cv.imshow('Taking Images', img)
...
def TrackImages():
    ...
    i = 0
    j = 0
    recognizer = cv.face.LBPHFaceRecognizer_create()  # cv.createLBPHFaceRecognizer()
    ...
    while True:
        ...
        for (x, y, w, h) in faces:
            ...
            if (conf < 50):
                ...
                Id = 'Unknown'
                bb = str(Id)
            cv.putText(im, str(bb), (x, y + h), font, 1, (255, 255, 255), 2)
        cv.imshow('Taking Attendance', im)
